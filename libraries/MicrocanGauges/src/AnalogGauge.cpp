...
AnalogGauge::AnalogGauge(String label, String units, float low, float high)
{
...
void AnalogGauge::Update(float value)
{
  ...
  float thick = m_r / 4.9;
  float norm = (m_oldValue - m_low) / (m_high - m_low);
  ...
  m_canvas->fillTriangle(x0, y0, x4, y4, x1, y1, TFT_WHITE);
    ...
    for (int i = 0; i <= 4; i++)
    {
      ...
      float mark = m_high - (m_high - m_low) / 4 * i;
  ...
  m_canvas->fillTriangle(x0, y0, x4, y4, x1, y1, TFT_RED);
