...
void loop() {
  ...
  if (M5.BtnB.wasClicked()) {
    filter = !filter;
    if (!filter) {
      M5.Lcd.printf("Filtering deactivated\n");
    } else {
      M5.Lcd.printf("Filtering activated\n");
    }
  }
  ...
  while (ESP32Can.readFrame(rxFrame, 0)) {
    if (!filter || SubaruPassFilter(rxFrame.identifier, 1000)) {
      didSomething = true;
      M5.Lcd.printf("%06d DL %03d D %03d %03d %03d %03d %03d %03d %03d %03d\n", rxFrame.identifier, rxFrame.data_length_code,
